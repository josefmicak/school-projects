...
{
	if (move >= rows * cols + 1)// move == rows * cols
	{
		return(true);
	}
	...
	for (int i = 0; i < 8; i++)
	{
		...
		if (isMoveValid(board, rows, cols, newX, newY))
		{
			...
			setKnightPos(board, newX, newY, 0);//Odebiram kone z dane pozice, cyklus se opakuje
		}
	}

	return(false);//Ani jedno z 8 policek nebylo reseni
}
