...
namespace RestauraceORM.Databaze.mssql
{
    class StiznostTable : StiznostProxy
    {
        ...
        public static string SQL_SELECT = "SELECT Stiznost.Id_stiznosti, Stiznost.Id_zakaznika, Stiznost.Id_zamestnance, Stiznost.Datum, Stiznost.Typ_stiznosti, Stiznost.Vyrizena, " +
            "Zamestnanec.Jmeno, Zamestnanec.Prijmeni " +
            "FROM \"Stiznost\" " +
            "JOIN Zamestnanec ON Zamestnanec.Id_zamestnance = Stiznost.Id_zamestnance";
        public static string SQL_SELECT_ID = "SELECT Stiznost.Id_stiznosti, Stiznost.Id_zakaznika, Stiznost.Id_zamestnance, Stiznost.Datum, Stiznost.Typ_stiznosti, Stiznost.Vyrizena, " +
            "Zamestnanec.Jmeno, Zamestnanec.Prijmeni " +
            "FROM \"Stiznost\" " +
            "JOIN Zamestnanec ON Zamestnanec.Id_zamestnance = Stiznost.Id_zamestnance " +
            "WHERE Id_stiznosti = @id";
        public static string SQL_SELECT_BY_ZAK = "SELECT Stiznost.Id_stiznosti, Stiznost.Id_zakaznika, Stiznost.Id_zamestnance, Stiznost.Datum, Stiznost.Typ_stiznosti, Stiznost.Vyrizena, " +
            "Zamestnanec.Jmeno, Zamestnanec.Prijmeni " +
            "FROM \"Stiznost\" " +
            "JOIN Zamestnanec ON Zamestnanec.Id_zamestnance = Stiznost.Id_zamestnance " +
            "WHERE Stiznost.Id_zakaznika = @id";
        public static string SQL_INSERT = "INSERT INTO \"Stiznost\" VALUES (@id_stiznosti, @id_zakaznika, @id_zamestnance," +
            "@datum, @typ_stiznosti, @vyrizena)";
        public static string SQL_DELETE_ID = "DELETE FROM \"Stiznost\" WHERE Id_stiznosti = @id";
        public static string SQL_UPDATE = "UPDATE \"Stiznost\" SET Id_zakaznika = @id_zakaznika, Id_zamestnance = @id_zamestnance, Datum = @datum," +
        ...
        protected override Collection<Stiznost> select_by_zak(int idZakaznika, DatabaseProxy pDb = null)
        {
            Database db;
            if (pDb == null)
            {
        ...
        protected override int delete(int idStiznosti, DatabaseProxy pDb = null)
        {
            Database db;
            if (pDb == null)
            {
        ...
        protected override Stiznost max(DatabaseProxy pDb = null)
        {
            Database db;
            if (pDb == null)
            {
            ...
            Collection<Stiznost> sti = maxRead(reader);
            Stiznost stiznost = null;
            if (sti.Count == 1)
            {
                stiznost = sti[0];
        ...
        private static Collection<Stiznost> maxRead(SqlDataReader reader)
        {
            Collection<Stiznost> sti = new Collection<Stiznost>();
