...
namespace RestauraceORM.Databaze.mssql
{
    class ZakaznikTable : ZakaznikProxy
    {
        ...
        public static string SQL_SELECT = "SELECT Id_zakaznika, Jmeno, Prijmeni, Adresa, Telefonni_cislo, Neaktivni, Kategorie, V_blacklistu, Zrusen FROM \"Zakaznik\"";
        public static string SQL_SELECT_ID = "SELECT Id_zakaznika, Jmeno, Prijmeni, Adresa, Telefonni_cislo, Neaktivni, Kategorie, V_blacklistu, Zrusen  FROM \"Zakaznik\" WHERE Id_zakaznika = @id";
        public static string SQL_INSERT = "INSERT INTO \"Zakaznik\" VALUES (@id_zakaznika, @jmeno, @prijmeni, @adresa," +
            "@telefonni_cislo, @neaktivni, @kategorie, @v_blacklistu, @zrusen)";
        ...
        public static string SQL_UPDATE = "UPDATE \"Zakaznik\" SET Jmeno = @jmeno, Prijmeni = @prijmeni, Adresa = @adresa," +
            "Telefonni_cislo = @telefonni_cislo, Neaktivni = @neaktivni, Kategorie = @kategorie, V_blacklistu = @v_blacklistu, Zrusen = @zrusen WHERE Id_zakaznika = @id";
        public static string SQL_GET_ID = "SELECT Id_zakaznika, Jmeno, Prijmeni, Adresa, Telefonni_cislo, Neaktivni, Kategorie, V_blacklistu, Zrusen FROM \"Zakaznik\" WHERE Jmeno = @jmenoZak AND Prijmeni = @prijmeniZak";
        ...
        protected override Zakaznik get(string jmenoZak, string prijmeniZak, DatabaseProxy pDb = null)
        {
            Database db;
            if (pDb == null)
            {
            ...
            SqlCommand command = db.CreateCommand(SQL_GET_ID);
            command.Parameters.AddWithValue("@jmenoZak", jmenoZak);
            command.Parameters.AddWithValue("@prijmeniZak", prijmeniZak);
            SqlDataReader reader = db.Select(command);
        ...
        protected override DatabaseProxy sumarizaceAktivityZakazniku(DatabaseProxy pDb = null)
        {
            Database db;
            if (pDb == null)
            {
            ...
            SqlCommand command = db.CreateCommand("sumarizace_aktivity_zakazniku");
