...
namespace RestauraceORM.Databaze.mssql
{
    class ZamestnanecTable : ZamestnanecProxy
    {
        ...
        public static string SQL_GET_ID = "SELECT Id_zamestnance, Jmeno, Prijmeni, Adresa, Telefonni_cislo, Pohlavi, Datum_narozeni, Datum_nastupu, Pozice, Plat, Vyhozen FROM \"Zamestnanec\" WHERE Jmeno = @jmenoZam AND Prijmeni = @prijmeniZam";
        ...
        protected override Zamestnanec get(string jmenoZam, string prijmeniZam, DatabaseProxy pDb = null)
        {
            Database db;
            if (pDb == null)
            {
            ...
            command.Parameters.AddWithValue("@jmenoZam", jmenoZam);
            command.Parameters.AddWithValue("@prijmeniZam", prijmeniZam);
            SqlDataReader reader = db.Select(command);
        ...
        protected override DatabaseProxy vypocetEfektivnostiRidicu(DatabaseProxy pDb = null)
        {
            Database db;
            if (pDb == null)
            {
            ...
            SqlCommand command = db.CreateCommand("vypocet_efektivnosti_ridicu");
